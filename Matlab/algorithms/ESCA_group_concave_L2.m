function [mu,A,B,S,out] = ESCA_group_concave_L2(dataSets,dataTypes,lambdas,fun,opts)

% Suppose B_lr indicates the r-th column of the loading matrix B_l of the l-th data set
% X_l. Concave penalty is applied to the L2-norm of B_lr to induce group
% sparsity. Furthermore, elements in B_lr are regularized in the same way as the ridge regression.  
...
f_obj0 = 0;
g_obj0 = 0;
Sigmas0 = nan(nDataSets,R); % sigma_{lr} = ||b_{lr}||_2
for i = 1:nDataSets,
    dataType = dataTypes{i};
    columns_Xi = (sum(d(1:(i-1)))+1):sum(d(1:i));
	X_i        = X(:, columns_Xi);
	...
	for r = 1:R,
	    sigma0_ir    = norm(B0(columns_Xi,r),2); % sigma_{lr} = ||b_{lr}||_2
...
for k = 1:maxit
	...
	%--- update B ---
	B = nan(sumd,R);
	Sigmas    = nan(nDataSets,R);  % sigma_{lr} = ||b_{lr}||_2
    for i=1:nDataSets,
	    columns_Xi = (sum(d(1:(i-1)))+1):sum(d(1:i));
		...
		for r=1:R,
		    ...
		    omega_ir  = hfun_sg(sigma0_ir,gamma,1); % weights
			...
			B(columns_Xi,r) = B_ir;
			sigma_ir    = norm(B_ir,2); % sigma_{lr} = ||b_{lr}||_2
			Sigmas(i,r) = sigma_ir;
		end
    end
    ...
    for i = 1:nDataSets,
	    ...
	    for r = 1:R,
			sigma_ir    = Sigmas(i,r);
		    g_obj = g_obj + lambda_i*hfun(sigma_ir,gamma,1);
        end
    end
	obj = f_obj + g_obj;
 ...
 [tmp_total,tmp_PCs] = varExp_Gaussian(weighted_X,weighted_mu',A,weighted_B,W);
