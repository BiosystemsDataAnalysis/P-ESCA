function [mu,A,B,S,out] = ESCA_group_concave_L1(dataSets,dataTypes,lambdas,fun,opts)

% Suppose B_lr indicates the r-th column of the loading matrix B_l of the l-th data set
% X_l. Concave penalty is applied to the L1-norm of B_lr to induce group
% sparsity. Furthermore, elements in B_lr are regularized in the same way as the lasso penalty. 
...
f_obj0 = 0;
g_obj0 = 0;
Sigmas0 = nan(nDataSets,R); % sigma_{lr} = ||b_{lr}||_1
for i = 1:nDataSets,
    dataType = dataTypes{i};
    columns_Xi = (sum(d(1:(i-1)))+1):sum(d(1:i));
	X_i        = X(:, columns_Xi);
	...
	alpha_i    = alphas(i);
	weight_i   = d(i); % weight when L1 norm is used
	...
	for r = 1:R,
	    sigma0_ir    = sum(abs(B0(columns_Xi,r))); % sigma_{lr} = ||b_{lr}||_1
...
for k = 1:maxit
	...
	%--- update B ---
	B = nan(sumd,R);
	Sigmas    = nan(nDataSets,R);  % sigma_{lr} = ||b_{lr}||_1
    for i=1:nDataSets,
	    columns_Xi = (sum(d(1:(i-1)))+1):sum(d(1:i));
