function [dataSimulation] = dataSimulation_BBB(n,ds,SNRgc,SNRlc,SNRd,margProb,ntimes_noise,sparse_ratio,seed)
% The simulation of Bernoulli-Bernoulli-Bernoulli (B-B-B) data sets 
...
while (rejection == 0),
	...
	% simulate data sets
	[dataSimulation] = dataSimulation_BBB_pro(n,ds,SNRgc,SNRlc,SNRd,margProb,sparse_ratio);
	U_simu = dataSimulation.U_simu;
	D_simu = dataSimulation.D_simu;
	V_simu = dataSimulation.V_simu;
	E_simu = dataSimulation.E_simu;
	
	% test if all the components are ntimes larger of the noise
	for i = 1:length(SNRs)
